...
Metis and delta-Mem release artifacts live under ignored `artifacts/` paths.
For a host-specific layout, copy the registry to
`artifacts/assets.local.json` and pass that file with `--assets`; keep the
...
`artifacts/models/<basename>` and every directory in
